...
class AdminLessonsService {
  ...
  Future<void> upsert(LessonModel model) async {
    await _firestore.collection(collectionPath).doc(model.id).set(model.toMap());
  }

  Future<void> setLocked(String id, bool locked) async {
    await _firestore.collection(collectionPath).doc(id).update({'isLocked': locked});
  }

  Future<void> setActive(String id, bool active) async {
    await _firestore.collection(collectionPath).doc(id).update({'isActive': active});
  }
